...
Since the task is to rank **genes**, this method computes IC over the distribution of phenotypes in the gene-to-phenotype reference set. 
...
## 3. Pairwise Similarity Measures

### 3.1 Lin's Similarity
Lin's similarity normalizes the IC of the Most Informative Common Ancestor (MICA) by the IC of the individual terms:
$$Sim_{Lin}(t_1, t_2) = \frac{2 \times IC(MICA(t_1, t_2))}{IC(t_1) + IC(t_2)}$$

### 3.2 Resnik Similarity
Resnik's similarity is simply the IC of the Most Informative Common Ancestor:
...
- **Hits@k**: Percentage of cases where the true gene is in the top $k$ positions.
...
The Saudi cohort is further subdivided into its six original source datasets. The same evaluation methodology described above is applied independently to each sub-cohort to assess variability in phenotypic data quality and specificity across different studies.
